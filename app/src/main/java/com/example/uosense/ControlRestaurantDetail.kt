...
class ControlRestaurantDetail : AppCompatActivity(), MenuImagePicker {
    ...
    private suspend fun updateBusinessDays(parentRecyclerView: RecyclerView): Boolean {
        return try {
            ...
            val response = RetrofitInstance.restaurantApi.editBusinessDay(businessDayList)
        ...
        return try {
            ...
            for (menu in menuItems) {
                val response = if (menu.menuId == -1) {
                    ...
                    RetrofitInstance.restaurantApi.updateMenu(request)
                }
    ...
    private fun uploadRestaurantImages(restaurantId: Int) {
        CoroutineScope(Dispatchers.Main).launch {
            try {
                ...
                val response = RetrofitInstance.restaurantApi.uploadRestaurantImages(restaurantId, imageParts)
